    ...
    : IRequestHandler<ResolveDepotClosureCommand, ResolveDepotClosureResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (closure.Status == DepotClosureStatus.Processing)
        {
            ...
            if (!reclaimed)
            {
                throw new ConflictException(
                    "Yêu cầu đóng kho đang được xử lý bởi tiến trình khác. Vui lòng thử lại sau.");
            }

            logger.LogInformation(
                "ResolveDepotClosure recovered stuck Processing | ClosureId={ClosureId}",
                request.ClosureId);
        }
        else if (closure.Status != DepotClosureStatus.InProgress)
        {
            throw new ConflictException(
                $"Bản ghi đóng kho đang ở trạng thái '{closure.Status}' và không thể tiếp tục xử lý.");
        }
        else
        {
        ...
        if (depot.Status != DepotStatus.Closing)
            throw new ConflictException("Kho không ở trạng thái Closing, không thể tiếp tục xử lý đóng kho.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (reusableAvailable, reusableInUse) = await depotRepository.GetReusableItemCountsAsync(
            request.DepotId,
            cancellationToken);
        ...
        logger.LogInformation(
            "DepotClosureTransfer created | ClosureId={ClosureId} TransferId={TransferId}",
            closure.Id,
            transfer.Id);
        ...
        {
            logger.LogWarning(notifyEx, "Failed to notify target manager | TransferId={TransferId}", transfer.Id);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new ResolveDepotClosureResponse
        {
            ...
            Message = "Đã xử lý xong hàng tồn theo hình thức bên ngoài. Kho vẫn giữ trạng thái Closing và chờ xác nhận đóng kho."
        };
    }
}
