    ...
    : IRequestHandler<GetAlternativeDepotsQuery, GetAlternativeDepotsResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var cluster = await _sosClusterRepository.GetByIdAsync(request.ClusterId, cancellationToken)
            ?? throw new NotFoundException($"Không tìm th?y c?m v?i ID: {request.ClusterId}");

        if (!cluster.CenterLatitude.HasValue || !cluster.CenterLongitude.HasValue)
            throw new BadRequestException($"C?m {request.ClusterId} không có t?a d? tâm d? x?p h?ng kho thay th?.");
        ...
        if (latestSuggestion is null)
            throw new NotFoundException($"C?m {request.ClusterId} chua có g?i ý nhi?m v? t? AI.");
                ...
                && shortage.SelectedDepotId.Value != request.SelectedDepotId))
        {
            throw new BadRequestException(
                $"selectedDepotId={request.SelectedDepotId} không kh?p v?i kho chính trong g?i ý AI m?i nh?t c?a c?m {request.ClusterId}.");
        }
        ...
        IReadOnlyCollection<AlternativeDepotItemCoverageDto> itemCoverageDetails)
    {
        ...
        {
            return $"Kho này dáp ?ng toàn b? ph?n thi?u h?t ({coveredQuantity}/{totalMissingQuantity} don v?), cách tâm c?m {distanceLabel} km.";
        }
        ...
        return $"Kho này dáp ?ng m?t ph?n {coveredQuantity}/{totalMissingQuantity} don v?, còn thi?u {missingLabel}, cách tâm c?m {distanceLabel} km.";
    }
    ...
    {
        return itemId.HasValue ? $"v?t ph?m #{itemId.Value}" : "v?t ph?m chua rõ";
    }
