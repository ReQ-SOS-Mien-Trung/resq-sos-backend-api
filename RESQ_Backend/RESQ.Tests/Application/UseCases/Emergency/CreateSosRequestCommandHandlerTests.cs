...
public class CreateSosRequestCommandHandlerTests
{
    ...
    private sealed class StubUserRepo(UserModel? user) : IUserRepository
    {
        ...
        public Task<PagedResult<UserModel>> GetPagedForPermissionAsync(int pn, int ps, int? roleId = null, string? name = null, string? phone = null, string? email = null, CancellationToken ct = default) => Task.FromResult(new PagedResult<UserModel>([], 0, pn, ps));
