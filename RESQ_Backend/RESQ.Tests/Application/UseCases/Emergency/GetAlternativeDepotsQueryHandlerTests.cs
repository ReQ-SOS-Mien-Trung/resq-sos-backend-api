...
public class GetAlternativeDepotsQueryHandlerTests
{
    ...
    public async Task Handle_ReturnsRankedTopThreeAlternativeDepots()
    {
        ...
        Assert.Collection(
            ...
            depot =>
            {
                ...
                Assert.Contains("cover mot phan", depot.Reason, StringComparison.OrdinalIgnoreCase);
            });
    }
    ...
    private sealed class StubDepotRepository(IEnumerable<DepotModel> depots) : IDepotRepository
    {
        ...
        public Task AssignManagerAsync(DepotModel depot, CancellationToken cancellationToken = default) => Task.CompletedTask;
        public Task UnassignManagerAsync(DepotModel depot, CancellationToken cancellationToken = default) => Task.CompletedTask;
