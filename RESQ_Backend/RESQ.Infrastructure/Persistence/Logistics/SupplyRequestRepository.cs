...
public class SupplyRequestRepository(IUnitOfWork unitOfWork) : ISupplyRequestRepository
{
    ...
    public async Task UpdateStatusAsync(int id, string sourceStatus, string requestingStatus, string? rejectedReason, CancellationToken cancellationToken = default)
    {
        var entity = await _unitOfWork.GetRepository<DepotSupplyRequest>()
            ...
            ?? throw new NotFoundException($"Không tìm th?y yêu c?u cung c?p #{id}.");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (itemModelId, quantity) in items)
        {
            ...
            var itemModel = await _unitOfWork.GetRepository<ItemModelEntity>()
                ...
                ?? throw new NotFoundException($"v?t ph?m #{itemModelId} không t?n t?i trong h? th?ng.");
            ...
            if (itemModel.ItemType == "Reusable")
            {
                ...
                if (availableUnits.Count < quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): kho ngu?n ch? có {availableUnits.Count} don v? kh? d?ng, yêu c?u {quantity}.");
                ...
                foreach (var unit in unitsToReserve)
                {
                    ...
                    await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                    {
                        ...
                        Note           = $"Ð?t tr? {itemModel.Name} (S/N: {unit.SerialNumber}) cho yêu c?u #{supplyRequestId}",
                        CreatedAt      = now
                    });
                }
            }
            else
            {
                ...
                var inventory = await _unitOfWork.GetRepository<SupplyInventory>()
                    ...
                    ?? throw new BadRequestException(
                        $"Kho ngu?n không có v?t ph?m '{itemModel.Name}' (#{itemModelId}) trong t?n kho.");

                var available = (inventory.Quantity ?? 0) - (inventory.MissionReservedQuantity + inventory.TransferReservedQuantity);
                if (available < quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): t?n kho kh? d?ng ({available}) không d? so v?i yêu c?u ({quantity}).");
                ...
                await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                {
                    ...
                    SourceId               = supplyRequestId,
                    PerformedBy            = performedBy,
                    Note                   = $"Ð?t tr? {itemModel.Name} (#{itemModelId}) cho yêu c?u #{supplyRequestId}",
                    CreatedAt              = now
                });
            }
        }
        ...
        foreach (var (itemModelId, quantity) in items)
        {
            ...
            if (itemModel.ItemType == "Reusable")
            {
                ...
                if (reservedUnits.Count != quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): tìm th?y {reservedUnits.Count} don v? d?t tr?, " +
                        $"không kh?p v?i yêu c?u {quantity}. Quy trình có th? b? b? qua bu?c Accept.");
                ...
                foreach (var unit in reservedUnits)
                {
                    ...
                    await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                    {
                        ...
                        SourceId       = supplyRequestId,
                        PerformedBy    = performedBy,
                        Note           = $"Xu?t ti?p t? {itemModel.Name} (S/N: {unit.SerialNumber}) cho yêu c?u #{supplyRequestId}",
                        CreatedAt      = now
                    });
                }
            ...
            else
            {
                ...
                var inventory = await _unitOfWork.GetRepository<SupplyInventory>()
                    .GetByPropertyAsync(x => x.DepotId == sourceDepotId && x.ItemModelId == itemModelId, tracked: true)
                    ?? throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): không tìm th?y t?n kho t?i kho ngu?n. " +
                        "Quy trình có th? b? b? qua bu?c Accept.");

                var reserved = inventory.TransferReservedQuantity;
                if (reserved < quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): s? lu?ng d?t tr? ti?p t? ({reserved}) không d? so v?i yêu c?u ({quantity}). " +
                        "Quy trình có th? b? b? qua bu?c Accept.");

                if ((inventory.Quantity ?? 0) < quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): t?n kho ({inventory.Quantity ?? 0}) không d? so v?i yêu c?u ({quantity}).");
                ...
                if (lots.Count > 0)
                {
                    ...
                    foreach (var lot in lots)
                    {
                        ...
                        await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                        {
                            ...
                            SourceId               = supplyRequestId,
                            PerformedBy            = performedBy,
                            Note                   = $"Xu?t ti?p t? FEFO lô #{lot.Id} {itemModel.Name} (#{itemModelId}) SL {deduct} cho yêu c?u #{supplyRequestId}",
                            CreatedAt              = now
                        });
                    }
                    ...
                    if (remaining > 0)
                        throw new InvalidOperationException(
                            $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): không d? lô d? xu?t ti?p t? {quantity} don v?.");
                }
                else
                {
                    ...
                    await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                    {
                        ...
                        SourceId               = supplyRequestId,
                        PerformedBy            = performedBy,
                        Note                   = $"Xu?t ti?p t? {itemModel.Name} (#{itemModelId}) SL {quantity} cho yêu c?u #{supplyRequestId} (legacy – không có lô)",
                        CreatedAt              = now
                    });
                }
            }
        ...
        foreach (var (itemModelId, quantity) in items)
        {
            ...
            if (itemModel.ItemType == "Reusable")
            {
                ...
                if (inTransitUnits.Count != quantity)
                    throw new BadRequestException(
                        $"v?t ph?m '{itemModel.Name}' (#{itemModelId}): tìm th?y {inTransitUnits.Count} don v? dang v?n chuy?n, " +
                        $"không kh?p v?i yêu c?u {quantity}. Quy trình có th? b? b? qua bu?c Ship.");
                ...
                foreach (var unit in inTransitUnits)
                {
                    ...
                    await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                    {
                        ...
                        SourceId       = supplyRequestId,
                        PerformedBy    = performedBy,
                        Note           = $"Nh?n ti?p t? {itemModel.Name} (S/N: {unit.SerialNumber}) t?i kho #{requestingDepotId} t? yêu c?u #{supplyRequestId}",
                        CreatedAt      = now
                    });
                }
            }
            else
            {
                ...
                // T?o lô m?i cho hàng du?c ti?p t? vào kho
                ...
                await _unitOfWork.GetRepository<InventoryLog>().AddAsync(new InventoryLog
                {
                    ...
                    SourceId               = supplyRequestId,
                    PerformedBy            = performedBy,
                    Note                   = $"Nh?n ti?p t? {itemModel.Name} (#{itemModelId}) t? yêu c?u #{supplyRequestId}",
                    CreatedAt              = now
                });
            }
        }
