...
IMPORTANT SOS COVERAGE CONTRACT (STRICT):
- NEED_RESCUE_ACTIVITY_CONTRACT (STRICT): add RESCUE or EVACUATE only for victims with `need_rescue=true`. `target_person_ids` for RESCUE/EVACUATE must contain only person_id values whose `need_rescue=true`. Do not create RESCUE/EVACUATE for `need_rescue=false` or missing/null victims, even when they have `medical_issues`.
- Every SOS with victims `need_rescue=true` must have direct RESCUE or EVACUATE coverage when field rescue/evacuation is required. Every SOS with medical fields (`medical_issues`, `severity`, or `is_injured=true`) must have MEDICAL_AID coverage when medical field work is required.
...
IMPORTANT SOS COVERAGE CONTRACT (STRICT):
- NEED_RESCUE_ACTIVITY_CONTRACT (STRICT): final RESCUE/EVACUATE activities may target only victims with `need_rescue=true`. Remove or rewrite any RESCUE/EVACUATE activity that targets `need_rescue=false` or missing/null victims. Preserve MEDICAL_AID for victims with `medical_issues`, `severity`, or `is_injured` even when `need_rescue=false`.
- Every SOS in SOS_REQUESTS_DATA must be covered by the minimal applicable final activity with sos_request_id exactly matching that SOS: DELIVER_SUPPLIES for consumables, MEDICAL_AID for medical fields, and RESCUE/EVACUATE only for victims with `need_rescue=true`.
